...
const map = (array, callback) => {
  ...
  array.forEach((ele) => result.push(callback(ele)));
...
const intersection = (arrays) => {
  return arrays.reduce((acc, curr) => {
    ...
    return acc.filter((ele) => curr.indexOf(ele) >= 0);
  }, []);
};
...
const union = (arrays) => {
  return arrays.reduce((acc, curr) => {
    ...
    return [...acc, ...curr.filter((ele) => acc.indexOf(ele) < 0)];
  }, []);
};
...
const functionValidator = (funcArr, input, output) => {
  return funcArr.filter((func) => func(input) === output);
};
...
//Re Write this
const curriedAddTwoNumsToFive1 = (num1) => (num2) => num1 + num2 + 5;

console.log(curriedAddTwoNumsToFive1(6)(7)); // should log 18
